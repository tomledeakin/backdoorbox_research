#!/bin/bash
#SBATCH --job-name=cifar10_adaptive_blend                         # Job name
#SBATCH --output=cifar10_scripts/cifar10_adaptive_blend_output.log # Output file
#SBATCH --error=cifar10_scripts/cifar10_adaptive_blend_error.log   # Error file
...
echo "Creating poisoned training set for adaptive_blend on cifar10..."
python create_poisoned_set.py -dataset=cifar10 -poison_type=adaptive_blend -poison_rate=0.01

# Step 2: Train the model on the poisoned dataset
echo "Training the model on the poisoned dataset..."
python train_on_poisoned_set.py -dataset=cifar10 -poison_type=adaptive_blend -poison_rate=0.01

# Step 3: Test the backdoor model
echo "Testing the backdoor model..."
python test_model.py -dataset=cifar10 -poison_type=adaptive_blend -poison_rate=0.01
...
python other_defense.py -defense=IBD_PSC -dataset=cifar10 -poison_type=adaptive_blend -poison_rate=0.01

echo "Experiment for cifar10 with adaptive_blend completed."
