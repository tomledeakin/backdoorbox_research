#!/bin/bash
#SBATCH --job-name=gtsrb_badnet                         # Job name
#SBATCH --output=gtsrb_scripts/gtsrb_badnet_output.log  # Output file
#SBATCH --error=gtsrb_scripts/gtsrb_badnet_error.log    # Error file
...
echo "Creating poisoned training set for badnet on GTSRB..."
...
python visualize.py -method=pca -dataset=gtsrb -poison_type=badnet -poison_rate=0.01
python visualize.py -method=tsne -dataset=gtsrb -poison_type=badnet -poison_rate=0.01
python visualize.py -method=oracle -dataset=gtsrb -poison_type=badnet -poison_rate=0.01

# Step 5: Apply IBD_PSC defense method
echo "Applying IBD_PSC defense method..."
python other_defense.py -defense=IBD_PSC -dataset=gtsrb -poison_type=badnet -poison_rate=0.01

echo "Experiment for GTSRB with badnet completed."
